document.addEventListener('DOMContentLoaded', function() {
    ...
    // Utilities & Visual Effects Module
    const setupUtilitiesAndEffects = () => {
        ...
        if (window.Notiflix) { 
            setInterval(() => { 
                const messages = [
                    "<strong>¡Éxito!</strong> Alguien acaba de desbloquear el acceso de por vida.", 
                    "<strong>¡Aprovecha!</strong> Un jugador acaba de asegurar el acceso de por vida.", 
                    "<strong>¡Imperdible!</strong> Otro jugador acaba de garantizar el acceso permanente."
                ];
            ...
            document.querySelectorAll(".purchase-link").forEach(link => {
                if (link.href && link.href.startsWith('http')) {
                    try {
                        ...
                        console.error("URL de checkout no válida:", link.href);
                    }
                }
            });
        }
    ...
    const setupGamification = () => {
        ...
        const updateGamifiedProgress = () => {
            ...
            if (percentage === 100 && !journeyCompleted) {
                ...
                setTimeout(() => {
                    if(window.Notiflix) {
                        Notiflix.Notify.Success('<strong>¡OFERTA DESBLOQUEADA!</strong> Demostraste tu nivel. Mira el plan especial.', { plainText: false, timeout: 5000, fontSize: '16px', width: '320px' });
                    }
        ...
        window.completeTask = (taskName) => {
            ...
            let achievementText = '';
            if (taskName === 'activated') achievementText = '<strong>LOGRO:</strong> ¡Panel activado!';
            if (taskName === 'testedAim') achievementText = '<strong>LOGRO:</strong> ¡Mira de élite!';
            if (taskName === 'optimized') achievementText = '<strong>LOGRO:</strong> ¡Dispositivo optimizado!';
    ...
    const setupCarousel = () => {
        ...
        const feedbackImages = [
            "https://i.imgur.com/NC8sgSy.png",
            "https://i.imgur.com/UajvZM1.png",
            "https://i.imgur.com/6LpvOAk.png",
            "https://i.imgur.com/Pd7spkl.png",
            "https://i.imgur.com/3GIql6p.png",
            "https://i.imgur.com/pRBfwHH.png",
            "https://i.imgur.com/jHHTrMH.png",
            "https://i.imgur.com/v7VueRe.png",
            "https://i.imgur.com/rWUmPO3.png",
            "https://i.imgur.com/66Z2P2J.png"
        ];
        let currentSlide = 0;
        ...
        const renderCarousel = () => { 
            ...
            carouselContent.innerHTML = feedbackImages.map(src => `
                <div class="flex-shrink-0 p-2" style="width: ${slideWidthPercent}%">
                    <img src="${src}" class="w-full h-auto rounded-lg" loading="lazy" alt="Opiniones de jugadores" />
                </div>
            `).join("");
    ...
    const setupFAQ = () => {
        ...
        const faqData = [
            {
                question: "¿El Panel FFH4X es seguro?",
                answer: "¡Sí! Nuestros paneles son 100% seguros, probados y funcionan sin poner en riesgo tu cuenta."
            },
            {
                question: "¿Funciona en todos los celulares?",
                answer: "Sí, el Panel FFH4X es compatible con todos los dispositivos Android y iOS."
            },
            {
                question: "¿Cómo recibo el acceso después de la compra?",
                answer: "La entrega es 100% inmediata. En cuanto se confirma tu pago, recibes automáticamente el enlace de descarga del panel y todas las instrucciones de instalación tanto por correo electrónico como por WhatsApp."
            },
            {
                question: "¿Tengo garantía?",
                answer: "¡Sí! Te ofrecemos una garantía de 7 días. Si no ves una mejora clara en tu jugabilidad o no quedas satisfecho, te devolvemos tu dinero."
            }
        ];
        ...
        const renderFaq = () => { 
            faqContainer.innerHTML = faqData.map((item, index) => `
                <div class="bg-gray-900 border border-gray-800 rounded-lg overflow-hidden transition-all duration-300 ${openFaq===index ? "border-red-500/50" : ""}">
                    <button data-faq-index="${index}" class="faq-toggle w-full flex justify-between items-center text-left p-6 cursor-pointer hover:bg-gray-800/50">
                        <h4 class="font-semibold text-lg text-white pr-4">${item.question}</h4>
                        <div class="flex-shrink-0 w-8 h-8 flex items-center justify-center bg-gray-800/50 rounded-full">
                            <span class="text-red-500 transition-transform duration-300 ${openFaq===index ? "rotate-180" : ""}">
                    ...
                    <div class="overflow-hidden transition-all duration-500 ease-in-out" style="max-height: ${openFaq===index ? "200px" : "0px"}">
            ...
            document.querySelectorAll(".faq-toggle").forEach(btn => { 
                btn.addEventListener("click", e => { 
                    const idx = parseInt(e.currentTarget.dataset.faqIndex); 
    ...
    const setupTracking = () => {
        document.querySelectorAll('.purchase-link').forEach(link => {
            link.addEventListener("click", function(event) {
                ...
                const contentId = `supersensi_${planName.toLowerCase().replace(/ /g, '_')}`;
    ...
    const setupAppSimulator = () => {
                ...
                if(aiResultDiv) {
                    ...
                    aiResultDiv.innerHTML = `
                        <p class="text-yellow-400 text-center font-bold">
                            Función no disponible en la versión de demostración.<br>
                            <span class="font-normal text-xs text-gray-300">
                                Disponible solo en la versión completa.
                            </span>
                        </p>`;
                }
            });
        ...
        if(applyOptimizationBtn) {
            applyOptimizationBtn.addEventListener('click', () => {
                ...
                applyOptimizationBtn.innerHTML = `<i class="fa-solid fa-spinner spin-loader"></i> Optimizando...`;
                applyOptimizationBtn.disabled = true;
                setTimeout(() => {
                    applyOptimizationBtn.innerHTML = `<i class="fa-solid fa-check"></i> ¡Optimizado!`;
